...
**Current day:** Day 27 — complete and verified. (Week 5 — proof/polish — day 3 of 6.)
**Last session:** 2026-09-14 — Day 27 HTML report generator.
**Completed:** D1–D27. `apiguard/report/generator.py` (`render_report`/`write_report`) + `apiguard/report/template.html` (jinja2, self-contained inline CSS, opens offline). `scan --report out.html` wired into the CLI. Report surfaces per-finding evidence + curl, and for AI-adjudicated findings the full oracle trace (model/seed/temp, the confidence signals as bars, prompt + raw response). Autoescaping ON (untrusted response bodies render inert — tested). Visually verified in-browser on the 8-finding Full scan. 131 tests pass.
**In progress:** nothing.
**Blocked / broken:** nothing.
**Next action:** Day 28 — Streamlit dashboard (`dashboard.py`), cassette-backed so the full demo runs offline (wifi off). Done when: full demo runs with wifi off. Reuse `cassettes/vampi/` (replay) + the saved arm JSONs + the HTML report; the dashboard is a thin consumer (invariant 1). NOTE Streamlit is in the plan's cut order (cut to a CLI demo if time-pressed) — but Week 5 is on schedule, so build it.
