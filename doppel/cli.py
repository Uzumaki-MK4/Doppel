"""Doppel command-line interface (Typer).
...
from doppel.core.http_engine import Cassette
from doppel.core.models import Endpoint, ScanResult
from doppel.core.scope import ScopeError
from doppel.core.spec_parser import load_spec, parse_spec
from doppel.settings import load_settings

app = typer.Typer(
    add_completion=False,
    help="Doppel — AI-powered API authorization scanner.",
)
console = Console()
...
@app.callback()
def _root() -> None:
    """Doppel — AI-powered API authorization scanner.

    A root callback keeps the subcommand style (`doppel parse ...`,
    `doppel scan ...`).
    """
...
def _write_report(out: str, result: ScanResult) -> None:
    ...
    from doppel.report.generator import write_report
