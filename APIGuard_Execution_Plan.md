...
The important design rule: **the engine must be a library, not a script.** Everything lives in a package called `apiguard`. The CLI imports it. Streamlit imports it. Your benchmark harness imports it. You write the logic once and get three faces for free.

```
                    +------------------+
                    |  apiguard/core   |  <- all the real logic lives here
                    +--------+---------+
             +---------------+---------------+
             |               |               |
      +------v-----+  +------v------+  +----v--------+
...
| **3** | `core/spec_parser.py`. Parse VAmPI's spec into `list[Endpoint]`. Handle `$ref`, path params, query params, request bodies, security schemes. | `apiguard parse <url>` prints a table of every endpoint with its parameters. |
...
| **6** | `cli.py` skeleton with `typer`, `rich` progress bar. Wire days 3-5 into one `apiguard scan --dry-run` command. | One command parses the spec, logs in both users, and touches every endpoint. |
...
| **15** | `ai/payload_gen.py`. Wire generated payloads into the injection scanner as an alternative payload source, switchable via `--payloads {static,ai,both}`. | `apiguard scan --payloads ai` runs end to end. |
...
| **27** | `report/generator.py` + `template.html`. Findings grouped by severity, OWASP mapping, full request/response evidence, the AI's reasoning trace per finding, copy-paste `curl` repro. | `apiguard scan --report out.html` produces something you would be happy to show. |
